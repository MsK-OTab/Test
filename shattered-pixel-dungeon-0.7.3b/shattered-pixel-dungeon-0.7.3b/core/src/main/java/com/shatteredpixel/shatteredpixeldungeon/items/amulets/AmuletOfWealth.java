...
public class AmuletOfWealth extends Amulet {
	...
	public static float dropChanceMultiplier( Char target ){
		return (float)Math.pow(1.2, 2 * getBonus(target, Wealth.class) - 1 );
	}
	
	public static ArrayList<Item> tryForBonusDrop(Char target, int tries ){
		if (2 * getBonus(target, Wealth.class) - 1 <= 0) return null;
	...
	public static Item genRareDrop(){
		...
		if (roll < 0.3f){ //30% chance
			...
			Item result = Random.Int(2) == 0 ? Generator.random(Generator.Category.AMULET) : Generator.random(Generator.Category.RING);
			result.cursed = false;
	...
	private static float dropProgression( Char target, int tries ){
		return tries * (float)Math.pow(1.2f, 2 * getBonus(target, Wealth.class) - 1);
	}

	public class Wealth extends AmuletBuff {
