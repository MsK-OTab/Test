...
public class WndJournal extends WndTabbed {
	...
	public static class AlchemyTab extends Component {
		...
		private static final int[] spriteIndexes = {12, 14, 9, 10, 11, 13, 15, 16, 17};
	...
	private static class CatalogTab extends Component{
		...
		private static final int AMULET_IDX = 3;
		private static final int RING_IDX   = 4;
//		private static final int ARTIF_IDX  = 4;
		private static final int POTION_IDX = 5;
		private static final int SCROLL_IDX = 6;
		
		private static final int spriteIndexes[] = {1, 2, 5, 6, 7, 11, 13};
		...
		private void updateList() {
			...
			if (currentItemIdx == WEAPON_IDX) {
				...
				for (Class<? extends Item> cls : itemClasses) known.put(cls, true);
			} else if (currentItemIdx == AMULET_IDX) {
				itemClasses = new ArrayList<>(Catalog.AMULETS.items());
				for (Class<? extends Item> cls : itemClasses) known.put(cls, Amulet.getKnown().contains(cls));
