...
public class ScrollOfDivination extends ExoticScroll {
	...
	public void doRead() {
		...
		HashSet<Class<? extends Amulet>> amulets = Amulet.getUnknown();
		HashSet<Class<? extends Ring>> rings = Ring.getUnknown();
		
		int total = potions.size() + scrolls.size() + rings.size() + amulets.size();
			...
			try {
				switch (Random.chances(probs)) {
					...
					case 3:
						...
						probs[3]--;
						Amulet a = Random.element(amulets).newInstance();
						a.setKnown();
						IDed.add(a);
						amulets.remove(a.getClass());
						break;
				}
			} catch (Exception e) {
