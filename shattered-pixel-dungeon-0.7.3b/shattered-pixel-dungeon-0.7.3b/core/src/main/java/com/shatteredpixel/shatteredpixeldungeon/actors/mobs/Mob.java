...
public abstract class Mob extends Char {
	...
	public void rollToDropLoot(){
		...
		lootChance *= AmuletOfWealth.dropChanceMultiplier( Dungeon.hero );
		...
		//changed to amulet of wealth logic
		if (Amulet.getBonus(Dungeon.hero, AmuletOfWealth.Wealth.class) > 0) {
			int rolls = 1;
			if (properties.contains(Property.BOSS)) rolls = 15;
			else if (properties.contains(Property.MINIBOSS)) rolls = 5;
			ArrayList<Item> bonus = AmuletOfWealth.tryForBonusDrop(Dungeon.hero, rolls);
			if (bonus != null && !bonus.isEmpty()) {
				for (Item b : bonus) Dungeon.level.drop(b, pos).sprite.drop();
				if (AmuletOfWealth.latestDropWasRare){
					new Flare(8, 48).color(0xAA00FF, true).show(sprite, 3f);
					AmuletOfWealth.latestDropWasRare = false;
				} else {
